...
server.post('/api/insert', upload.single('File'), (req, res) => {
  db.connect(() => {
    ...
    if(authData){
      ...
      const sqlInsert = `INSERT INTO surat (ID_User, Klasifikasi, No_Urut, Waktu, Tujuan, Nomor_Surat, Perihal, Kaitan, Keterangan, File) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
      const values = [ID_AdminValue, Klasifikasi, No_Urut, Waktu, Tujuan, Nomor_Surat, Perihal, Kaitan, Keterangan, File,];
...
server.get('/api/suratMasuk/:tahun', (req, res) => {
  ...
  const sqlSelect = "SELECT * FROM surat WHERE Klasifikasi = 'Surat Masuk' AND YEAR(Waktu) = ? ORDER BY No_Urut ASC";
...
//api menampilkan data surat
server.get('/jenis/:id/:tahun', (req, res) => {
  const Tahun = req.params.tahun;
  const ID_Klasifikasi = req.params.id;
  const sqlSelect = "SELECT * FROM surat WHERE Klasifikasi = ? AND YEAR(Waktu) = ? ORDER BY No_Urut ASC";
  db.query(sqlSelect, [ID_Klasifikasi, Tahun], (err, result) => {
      res.send(result);
  });
});
...
server.get('/api/suratKeluar/:tahun', (req, res) => {
  const Tahun = req.params.tahun;
  const sqlSelect = "SELECT * FROM surat WHERE Klasifikasi = 'Surat Keluar' AND YEAR(Waktu) = ? ORDER BY No_Urut ASC";
  db.query(sqlSelect, [Tahun], (err, result) => {
      res.send(result);
  });
});
...
server.get('/api/surat/all', (req, res) => {
  const sqlSelect = "SELECT SUM(CASE WHEN Klasifikasi = 'Surat Masuk' THEN 1 ELSE 0 END) AS jumlah_surat_masuk, SUM(CASE WHEN Klasifikasi = 'Surat Keluar' THEN 1 ELSE 0 END) AS jumlah_surat_keluar FROM surat";
  db.query(sqlSelect, (err, result) => {
      res.send(result);
  });
});
